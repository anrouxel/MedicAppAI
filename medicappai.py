...
if __name__ == '__main__':
    ...
    sentence = "Doliprane 1000 mg, 1 fois par jour pendant 3 jours"
    ...
    if use_cuda:
        traced_script_module.save("model_gpu.pt")

        model_cpu = torch.jit.load("model_gpu.pt", map_location=torch.device('cpu'))

        mask_cpu = text['attention_mask'].to(torch.device('cpu'))
        input_id_cpu = text['input_ids'].to(torch.device('cpu'))

        traced_script_module_cpu = torch.jit.trace(model_cpu, (input_id_cpu, mask_cpu))
        traced_script_module_cpu.save("model_cpu.pt")
    else:
